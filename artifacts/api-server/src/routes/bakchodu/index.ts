...
- Tu ek powerful AI hai jo Gemini 2.0 Flash model par kaam karta hai
- Photos aur images bhi dekh sakta hai aur unka detailed jawab de sakta hai
...
router.get("/info", (_req, res) => {
  res.json({
    ...
    model: GEMINI_MODEL,
    version: "2.0.0",
    description: "Ek mast AI assistant jo bilkul dost ki tarah baat karta hai! Photos bhi dekh sakta hai. MR. SURAJ SIR ki mehnat ka nateeja.",
    capabilities: ["text", "vision", "code", "math", "streaming"],
  });
});
  ...
  try {
    ...
    const contents = [
      ...history.map((h) => ({
        ...
        parts: buildUserParts(h.content, h.images),
      })),
      {
        role: "user" as const,
        parts: buildUserParts(message, images),
      },
    ];
